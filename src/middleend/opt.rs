...
/// Perform def-use analysis over values, and
/// remove all unused values in a function.
/// In terms of memory accesses (i.e. loads and stores),
/// a naive over-approximation is used: accesses 
/// are always associated with alloc-ed address if
/// possible, and are assumed to refer to anywhere if
/// it comes from a loaded address.
...
impl FunctionPass for ElimUnusedValue {
    ...
    fn run_on(&mut self, _func: Function, data: &mut FunctionData) {
        ...
        // - Function calls
        // - Effective stores; a store is effective if it is possibly read
        // before another dominating store.

        type State = HashSet<BasicBlock>;

        // Perform liveliness analysis over all alloc-ed addresses.
        // Ideally the initial state should contain all global allocs.
        // For now we do only local analysis.
        let mut to_visit = VecDeque::new();
        to_visit.extend(data.layout()
            .bbs()
            .iter()
            .filter(|(_, bb_data)| {
                let last_inst = bb_data.insts().back_key().expect("[OPT] empty BB");
                if matches!(data.dfg().value(*last_inst).kind(), ValueKind::Return(_)) {
                    return true;
                }
                false
            })
            .map(|(bb, _)| (*bb, State::new()))    
        );
